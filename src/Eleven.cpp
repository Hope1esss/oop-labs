...
Eleven &Eleven::operator+=(const Eleven &other)
{
   ...
   {
      int digit1 = (i < size_) ? elevenToDecimal(data_[size_ - 1 - i]) : 0;
      int digit2 = (i < other.size_)
                       ? elevenToDecimal(other.data_[other.size_ - 1 - i])
                       : 0;

      int sum = digit1 + digit2 + carry;
      carry = sum / 11;
      result[maxSize - i] = decimalToEleven(sum % 11);
   }

   if (carry > 0)
   {
      result[0] = decimalToEleven(carry);
   }
   else
   {
      result++;
...
Eleven &Eleven::operator-=(const Eleven &other)
{
   ...
   for (size_t i = 0; i < size_; i++)
   {
      int currentDigit = elevenToDecimal(data_[size_ - 1 - i]) - borrow;
      ...
      data_[size_ - 1 - i] = decimalToEleven(currentDigit - otherDigit);
   }
...
bool Eleven::operator<(const Eleven &other) const
{
   ...
   for (size_t i = 0; i < size_; i++)
   {
      int digit1 = elevenToDecimal(data_[i]);
      int digit2 = elevenToDecimal(other.data_[i]);
